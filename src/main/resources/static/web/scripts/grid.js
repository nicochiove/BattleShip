...
*/
const createGrid = function(size, element, id, isSalvoGrid){
    ...
    for(let i = 0; i < size; i++){
        ...
        for(let j = 0; j < size; j++){
            ...
            if(i > 0 && j > 0){
                ...
                if(isSalvoGrid){
                  cell.addEventListener('click',function(event) {shootHere(event)})      
                }
            }
...
createGrid(11,document.getElementById('grid_salvo'), 'salvo',true)
...
function shootHere(ev){
  let celda= ev.target
  limpiarDisplay()
  
  if(celda.classList.contains('fireable')){
    if(document.querySelector('.remainingSalvo') && !celda.classList.contains('aimedSalvo')){
      celda.classList.toggle('aimedSalvo')
      document.querySelector('.remainingSalvo').remove()
    }else{
      if(celda.classList.contains('aimedSalvo')){
        celda.classList.toggle('aimedSalvo')
        ...
        console.log('ya tiraste todo lo que pode che vo')
        document.querySelector('#display p').innerText= 'No more ammo for this turn captain!!!'
